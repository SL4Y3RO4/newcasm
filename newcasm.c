		  ...
		  //print
			...
			for(int i = 0; i < namec; i++) {
				if(strcmp(varnames[i], varname) == 0) {
				    check_pos = i;
					switch(type[i]) {
						case INT: printf("%d", numsc[check_pos]); break;
						case STR: printf("%s", strlist[check_pos]); break;
						case BOOL: printf("%s", boolList[check_pos]); break;
						case CHR: printf("%c", takechr[check_pos]); break;
					}
